...
class plotSkewT(backgroundSkewT):
    ...
    def __init__(self, **kwargs):
        ...
        storm_motion.setText("Cursor de lectura")
        ...
        reset = QAction(self)
        reset.setText("Restablecer Skew-T")
    ...
    def getPlotTitle(self, prof_coll):
        ...
        modified_str = "; Modificado" if modified else ""
    ...
    def liftparcellevel(self, i):
        ...
        if i == 0:
            ...
            if i == -9999:
                depth, result = QInputDialog.getText(None, "Prof. de parcela (" + str(int(pres)) + "to __)",\
                                            "Prof. de capa media (mb):")
                try:
                    i = int(depth)
                except:
                    return
...
#           idx_pbot, idx_ptop = np.searchsorted(np.array([self.prof.dgz_pbot, self.prof.dgz_ptop]), self.prof.pres)
    ...
    def drawBarbs(self, prof, qp, color="#FFFFFF"):
        ...
        if self.interpWinds is False:
            ...
            for y in yvals:
                dd = wdir[i]
                ss = wspd[i]
                drawBarb( qp, self.barbx, y, dd, vv, '#FFFFFF', prof.latitude)
                i += 1
        else:
            ...
            for p, dd, ss in zip(pres, wdir, wspd):
                ...
                drawBarb( qp, self.barbx, y, dd, ss, color, prof.latitude)
        qp.setClipRect(self.clip)
    ...
    def draw_sig_levels(self, qp, plevel=1000, color="#FFFFFF"):
        ...
        if not tab.utils.QC(plevel):
            return
        xbounds = [37,45]
        z = tab.interp.hght(self.prof, plevel)
#        z = tab.utils.M2FT(tab.interp.hght(self.prof, plevel))
    ...
    def draw_parcel_levels(self, qp):
        ...
        if tab.utils.QC(lclp):
            ...
            qp.drawText(rect1, QtCore.Qt.TextDontClip | QtCore.Qt.AlignCenter, "NCA")
        # Plot LFC
        if tab.utils.QC(lfcp):
            ...
            qp.drawText(rect2, QtCore.Qt.TextDontClip | QtCore.Qt.AlignCenter, "NCL")
        # Plot EL
        if tab.utils.QC(elp) and elp != lclp:
            ...
            qp.drawText(rect3, QtCore.Qt.TextDontClip | QtCore.Qt.AlignCenter, "NE")
...
#           else:
    ...
    def drawTrace(self, data, color, qp, width=3, style=QtCore.Qt.SolidLine, p=None, stdev=None, label=True):
        ...
        if label is True:
            qp.setClipping(False)
            label = data[0] # JP: MANTENER TEMP EN C
#            label = (1.8 * data[0]) + 32.
            pen = QtGui.QPen(QtGui.QColor('#000000'), 0, QtCore.Qt.SolidLine)
            brush = QtGui.QBrush(QtCore.Qt.SolidPattern)
            qp.setPen(pen)
