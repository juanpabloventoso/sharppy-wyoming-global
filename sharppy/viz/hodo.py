...
class plotHodo(backgroundHodo):
    ...
    def setActiveCollection(self, pc_idx, **kwargs):
        ...
        self.plotData()
        self.update()
        self.setSRCenter() # AGREGADO POR JP: Partir de SRCenter
    ...
    def mouseReleaseEvent(self, e):
        if self.cursor_type == 'stormmotion' and not self.was_right_click:
            ...
            if self.track_cursor:
                ...
                qp.drawText(rect, QtCore.Qt.TextDontClip | QtCore.Qt.AlignLeft, "Cortante 0-6km: " + tab.utils.INT2STR(dir) + '/' + tab.utils.INT2STR(spd) + ' kts')
                ...
                if spd >= 70:
                    ...
                    supercell_type = "HP"
                qp.drawText(rect, QtCore.Qt.TextDontClip | QtCore.Qt.AlignLeft, "Viento rel. 9-11km: " + tab.utils.INT2STR(dir) + '/' + tab.utils.INT2STR(spd) + ' kts - (' + supercell_type + ')')
...
#           new_prof = type(self.prof).copy(self.prof, u=new_u, v=new_v)
    ...
    def mouseMoveEvent(self, e):
        ...
        if self.cursor_type == 'stormmotion' and self.track_cursor:
            ...
            if tab.utils.QC(etop) and tab.utils.QC(ebot):
                ...
                self.esrhReadout.setText('Efectivo: ' + tab.utils.INT2STR(esrh) + ' m2/s2')
            else:
                esrh = np.ma.masked
                self.esrhReadout.setText('Efectivo: ' + str(esrh) + ' m2/s2')
            ...
            self.srh1kmReadout.setText('Sup-1km: ' + tab.utils.INT2STR(srh1km) + ' m2/s2')
            self.srh3kmReadout.setText('Sup-3km: ' + tab.utils.INT2STR(srh3km) + ' m2/s2')
            ...
            dir, spd = tab.utils.comp2vec(u,v)
            self.plotBndy(dir)
            self.srh3kmReadout.setText('Bndy Mov.: ' + tab.utils.INT2STR(dir) + '/' + tab.utils.INT2STR(spd))
            self.srh3kmReadout.setFixedWidth(120)
            self.srh3kmReadout.move(self.brx-130, self.bry-30)
    ...
    def drawSMV(self, qp):
		...
		## TODO JP TODO CAMBIO LM Y RM, NO SE SI ES LA MEJOR FORMA IGUAL
        qp.drawText(rm_rect, QtCore.Qt.AlignCenter, rm_stuff + " LM")
        qp.drawText(lm_rect, QtCore.Qt.AlignCenter, lm_stuff + " RM")
    ...
    def drawCriticalAngle(self, qp):
        ...
        if tab.utils.QC(self.ptop) and tab.utils.QC(self.pbottom):
            ...
            if tab.utils.QC(rstu) and tab.utils.QC(lstu):
                ...
                qp.setFont(self.critical_font)
                offset = 10
                qp.drawText(rect, QtCore.Qt.AlignLeft, 'Angulo critico = ' + tab.utils.INT2STR(self.prof.critical_angle))
