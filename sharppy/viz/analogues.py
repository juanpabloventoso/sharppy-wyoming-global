...
class plotAnalogues(backgroundAnalogues):
    ...
    def drawSARS(self, qp, **kwargs):
        ...
        if self.matches[0] == []:
            ...
            match_str = '(' + num_matches + ' coincid. sueltas)'
            ...
            ## if there are more than 0 quality matches...
                ...
                for m in self.matches[0]:
                    ...
                    if type == 'TOR':
                        ...
                        size_str = str( format(size * 2.5, '.1f' ) ) + 'cm' # JP JP: PASADO A CM
                        if size >= 2.0:
                            pen.setColor(QtGui.QColor(RED))
                            qp.setPen(pen)
                        else:
