...
class backgroundKinematics(QtGui.QFrame):
    ...
    def draw_frame(self, qp):
        ...
        qp.drawText(rect2, QtCore.Qt.TextDontClip | QtCore.Qt.AlignCenter, 'Cort. (kt)')
        qp.drawText(rect3, QtCore.Qt.TextDontClip | QtCore.Qt.AlignCenter, 'Viento med.')
        qp.drawText(rect4, QtCore.Qt.TextDontClip | QtCore.Qt.AlignCenter, 'VRT')
        ## left column
        ## first block
        texts = ['Sup-1km', 'Sup-3km', 'Capa de ent. ef.',]
        for text in texts:
        ...
        self.ylast = y1
        ## second block
        texts = ['Sup-6km', 'Sup-8km','LCL-EL (Capa de nube)', 'Cortante efect. (EBWD)']
        y1 = self.ylast + self.tpad
        for text in texts:
            rect = QtCore.QRect(self.lpad, y1, x1, self.label_height)
            qp.drawText(rect, QtCore.Qt.TextDontClip | QtCore.Qt.AlignLeft, text)
        ...
        self.ylast = y1
        ## third block
        texts = ['Cortante BRN = ', 'Viento rel. 4-6km = ']
        y1 = self.ylast + self.tpad
        for text in texts:
            rect = QtCore.QRect(self.lpad, y1, x1, self.label_height)
            qp.drawText(rect, QtCore.Qt.TextDontClip | QtCore.Qt.AlignLeft, text)
        ...
        self.ylast = y1
        ## fourth block
        texts = ['Vectores de mov. de tormentas', 'Bunkers izq = ', 'Bunkers der = ', 'Cortante arriba = ', 'Cortante abajo = ']
        y1 = self.ylast + self.tpad
...
class plotKinematics(backgroundKinematics):
    ...
    def drawBarbs(self, qp):
        ...
        qp.drawText(rect0, QtCore.Qt.TextDontClip | QtCore.Qt.AlignCenter, 'AGL 1km & 6km\nBarbas de viento' )
        drawBarb(qp, origin_x, self.barby, self.prof.wind1km[0], self.prof.wind1km[1], '#AA0000', self.prof.latitude)
        drawBarb(qp, origin_x, self.barby, self.prof.wind6km[0], self.prof.wind6km[1], '#0A74C6', self.prof.latitude)
