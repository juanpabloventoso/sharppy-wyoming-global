...
def drawFullBarb(path, latitude):
    pos = path.currentPosition()
    if latitude < 0:
        path.lineTo(pos.x(), pos.y() - 10)
...
def drawHalfBarb(path, latitude):
    pos = path.currentPosition()
    if latitude < 0:
...
def drawBarb(qp, origin_x, origin_y, wdir, wspd, color, latitude):
    ...
    if wspd > 0:
        ...
        while wspd >= 50:
            drawFlag(path, latitude)
            wspd -= 50

        while wspd >= 10:
            drawFullBarb(path, latitude)
            wspd -= 10

        while wspd >= 5:
            drawHalfBarb(path, latitude)
            wspd -= 5
